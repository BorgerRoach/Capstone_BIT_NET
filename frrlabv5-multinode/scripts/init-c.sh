...
sudo nsenter -t $PID -n ip link set vxlan-ca name eth2
sudo nsenter -t $PID -n ip link set vxlan-ct name eth3

sudo nsenter -t $PID -n ip link set eth2 up
sudo nsenter -t $PID -n ip link set eth3 up

# C → A
sudo nsenter -t $PID -n ip addr add 10.0.2.1/31 dev eth2

# C → T
sudo nsenter -t $PID -n ip addr add 10.0.1.1/31 dev eth3
